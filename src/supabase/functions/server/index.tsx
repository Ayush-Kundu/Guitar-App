...
// Community post endpoint - using existing 'posts' table
app.post('/make-server-4ea82950/community-post', async (c) => {
  try {
    const { userId, userName, username, userLevel, avatar, content, timestamp } = await c.req.json();
    
    console.log('Community post received:', { userId, userName, content });
    
    // Validate required fields
    if (!userId || !userName || !content) {
      return c.json({ error: 'Missing required fields: userId, userName, and content are required' }, 400);
    }
    
    // Create the post data for the 'posts' table
    const postData = {
    ...
    // Store in the existing 'posts' table
    ...
    if (error) {
      ...
      return c.json({ error: 'Failed to create community post', details: error.message }, 500);
    }
    
    console.log('Community post created successfully in posts table:', postData.id);
