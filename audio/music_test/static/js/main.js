...
// Helper to safely get DOM element (returns null if not found)
function getEl(id) {
  return document.getElementById(id);
}

// Check if we're on the index.html page (DOM elements exist)
function hasUI() {
  return !!getEl('startBtn');
}
...
function initializeSettings() {
  ...
  // Only update UI if DOM elements exist (index.html page)
  if (!hasUI()) return;

  // Update UI with config values
  const el = (id) => getEl(id);
  if (el('instrument')) el('instrument').value = config.instrument || 'guitar';
  if (el('sensitivity')) el('sensitivity').value = config.sensitivity || 1.0;
  if (el('sensitivityValue')) el('sensitivityValue').textContent = config.sensitivity || 1.0;
  if (el('confidenceThreshold')) el('confidenceThreshold').value = config.confidence_threshold || 0.6;
  if (el('confidenceValue')) el('confidenceValue').textContent = config.confidence_threshold || 0.6;
  if (el('silenceThreshold')) el('silenceThreshold').value = config.silence_threshold || 0.005;
  if (el('silenceValue')) el('silenceValue').textContent = config.silence_threshold || 0.005;
  if (el('overlap')) el('overlap').value = config.overlap || 0.75;
  if (el('overlapValue')) el('overlapValue').textContent = config.overlap || 0.75;
  if (el('progression')) el('progression').checked = config.progression !== false;
  if (el('multiPitch')) el('multiPitch').checked = config.multi_pitch !== false;
  if (el('showFrequencies')) el('showFrequencies').checked = config.show_frequencies === true;
  if (el('showChroma')) el('showChroma').checked = config.show_chroma === true;
  if (el('debug')) el('debug').checked = config.debug === true;
  if (el('log')) el('log').checked = config.log === true;
}
...
function updateStatus(connected, text) {
  ...
  if (indicator) indicator.className = 'status-indicator ' + (connected ? 'connected' : 'disconnected');
...
function handleWebSocketMessage(data) {
  if (data.type === 'connected') {
    ...
    if (getEl('startBtn')) getEl('startBtn').disabled = false;
...
function updateChordDisplay(data) {
  const chordNameEl = getEl('chordName');
  const chordConfidenceEl = getEl('chordConfidence');
  const chordStabilityEl = getEl('chordStability');

  // Skip if DOM elements don't exist (React app handles its own UI)
  ...
  if (data && data.chord) {
    ...
    if (chordStabilityEl && data.stability !== undefined) {
    ...
    if (chordConfidenceEl) chordConfidenceEl.style.display = 'none';
    if (chordStabilityEl) chordStabilityEl.textContent = '';
  }
}

// Update notes display - only show the most confident note (first in array)
function updateNotesDisplay(notes) {
  const notesListEl = getEl('notesList');

  // Skip if DOM element doesn't exist
  if (!notesListEl) return;

  if (notes.length > 0) {
    // Only show the first (most confident) note
    const bestNote = notes[0];
    const [noteName, freq] = Array.isArray(bestNote) ? bestNote : [bestNote, null];
    notesListEl.innerHTML = `<span class="note-badge">${noteName}${freq ? ` (${freq.toFixed(0)}Hz)` : ''}</span>`;
  } else {
...
async function startRecording() {
  try {
    ...
    isRecording = true;
    if (getEl('startBtn')) getEl('startBtn').disabled = true;
    if (getEl('stopBtn')) getEl('stopBtn').disabled = false;
...
function stopRecording() {
  ...
  if (getEl('startBtn')) getEl('startBtn').disabled = false;
  if (getEl('stopBtn')) getEl('stopBtn').disabled = true;
  updateStatus(ws && ws.readyState === WebSocket.OPEN, 'Connected');
}

// Settings modal - only initialize if on index.html page
const modal = getEl('settingsModal');
const settingsBtn = getEl('settingsBtn');
const closeBtn = document.querySelector('.close');
const settingsForm = getEl('settingsForm');
const resetSettingsBtn = getEl('resetSettings');
...
// Only set up UI event listeners if we're on the index.html page
if (hasUI()) {
  // Update range value displays
  if (getEl('sensitivity')) {
    getEl('sensitivity').addEventListener('input', (e) => {
      if (getEl('sensitivityValue')) getEl('sensitivityValue').textContent = e.target.value;
    });
  }

  if (getEl('confidenceThreshold')) {
    getEl('confidenceThreshold').addEventListener('input', (e) => {
      if (getEl('confidenceValue')) getEl('confidenceValue').textContent = e.target.value;
    });
  }

  if (getEl('silenceThreshold')) {
    getEl('silenceThreshold').addEventListener('input', (e) => {
      if (getEl('silenceValue')) getEl('silenceValue').textContent = e.target.value;
    });
  }

  if (getEl('overlap')) {
    getEl('overlap').addEventListener('input', (e) => {
      if (getEl('overlapValue')) getEl('overlapValue').textContent = e.target.value;
    });
  }
    ...
    settingsForm.addEventListener('submit', (e) => {
      ...
      for (const [key, value] of formData.entries()) {
        if (key === 'progression' || key === 'multi_pitch' || key === 'show_frequencies' ||
          ...
          const elId = key === 'multi_pitch' ? 'multiPitch' :
            key === 'show_frequencies' ? 'showFrequencies' :
              key === 'show_chroma' ? 'showChroma' : key;
          const el = getEl(elId);
          config[key] = el ? el.checked : false;
        } else {
          config[key] = value;
        }
      }
  ...
  if (getEl('startBtn')) {
    getEl('startBtn').addEventListener('click', startRecording);
  }
  if (getEl('stopBtn')) {
    getEl('stopBtn').addEventListener('click', stopRecording);
  }
}
...
window.addEventListener('load', () => {
  initializeSettings();
  // Only disable start button if it exists (index.html page)
  if (getEl('startBtn')) {
    getEl('startBtn').disabled = true;
  }
  // Only connect WebSocket if on index.html (React app handles its own connection)
  if (hasUI()) {
    connectWebSocket();
  }
});
